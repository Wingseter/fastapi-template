...
async def access_token() ->str:
    return create_access_token("mail@gmail.com")

@pytest.fixture(scope="module")
async def mock_event() -> Event:
    new_event = Event(
        creator = "mail@gmail.com",
        title="Fastapi",
        image="https://blabla/image.png",
...
@pytest.mark.asyncio
async def test_post_event(default_client: httpx.AsyncClient, mock_event: str) -> None:
    payload = {
        "title"="Fastapi",
        "image"="https://blabla/image.png",
        "description"="We will be discussing"
...
@pytest.mark.asyncio
async def test_update_event(default_client: httpx.AsyncClient, mock_event: str) -> None:
    test_payload = {
        "title"="Fastapi",
    }
    ...
    response = await default_client.post(url, json=payload, headers=headers)

    assert response.status_code == 200
    assert response.json()["title"] == test_response["title"]
...
@pytest.mark.asyncio
async def test_get_events_again(default_client: httpx.AsyncClient, mock_event: Event) -> None:
    url = f"/event/{str(mock_event.id)}"
    response = await default_client.get(url)
